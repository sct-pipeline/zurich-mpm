...
label_if_does_not_exist(){
  ...
  if [ -e "${PATH_SEGMANUAL}/${file}_labels-manual.nii" ]; then
    ...
    sct_label_vertebrae -i ${file}.nii -s ${file_seg}.nii -c t2 -qc ${PATH_QC} -qc-subject ${SUBJECT}
...
segment_if_does_not_exist $file_r1 "t1"
file_seg=$FILESEG
# Create labels in the cord at C2 and C3 mid-vertebral levels (only if it does not exist)
label_if_does_not_exist $file_pd $file_seg
file_label=$FILELABEL
# Register to template
# Note: for such a small rostro-caudal distance, we can use -ref subject
sct_register_to_template -i $file_r1.nii -s $file_seg.nii -l $file_label.nii -c t1 -ref subject -param step=1,type=seg,algo=centermassrot:step=2,type=im,algo=syn,iter=5,slicewise=1,metric=CC,smooth=0 -qc $PATH_QC
# Warp template
sct_warp_template -d $file_r1.nii -w warp_template2anat.nii.gz -qc $PATH_QC
